...
async function post(req, res) {
    ...
    if (req.body && req.body.title && req.body.content && req.body.category_id) {
        let slug;
        let counter = 1;
        ...
        while (true) {
            ...
            if (await Article.query().where('slug', slug).first()) {
                counter++;
                continue;
            }
            break;
        ...
        let article = await Article.query().insertAndFetch({ ...req.body, slug });

        // Si todo ha salido bien, devolvemos los datos insertados
        if (article) {
            return res.json({ status: 200, article });
        }
    }
